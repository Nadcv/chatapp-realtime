...
**Isto não é GPS real** — assume que a viagem está a decorrer exatamente conforme o horário, sem atrasos nem paragens não planeadas. Cada comboio no mapa mostra isso claramente no popup ("posição estimada pelo horário — não é GPS ao vivo"), com um ícone de comboio (🚂), uma linha tracejada a marcar a rota completa dessa viagem (todas as paragens, não só o troço atual) e um marcador por paragem (🟢 origem, 🔸 intermédia, 🔴 destino) com a hora do horário de cada uma. Atualiza a cada 30 segundos enquanto o interruptor estiver ligado (cálculo local, sem custo de pedidos a nenhuma API externa). Endpoint: `/api/trains/positions-estimated`.
