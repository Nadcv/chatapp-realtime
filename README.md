...
- **🚌 Autocarros** — posição ao vivo de cada autocarro da Carris Metropolitana (Área Metropolitana de Lisboa), via API oficial gratuita e sem chave. Tem um alternador "Lisboa (ao vivo)" / "Guimarães (horários)" / "Porto" / "Madrid (EMT)" — Guimarães mostra horários programados da GUIMABUS (via GTFS aberto); Porto mostra Metro do Porto (horários, via GTFS) e STCP (chegadas em tempo real); Madrid mostra horários programados da EMT (via GTFS aberto, ver secção própria abaixo). Só Lisboa (Carris) e a STCP têm posição/chegada ao vivo — o resto é horário programado.
