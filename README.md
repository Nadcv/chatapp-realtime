...
- **☁️ AEMET OpenData (meteorologia)** — a mais fácil das três. É uma API REST **gratuita** e oficial da Agência Estatal de Meteorologia espanhola: só pedes uma API key inserindo o teu email em `opendata.aemet.es`, recebes a chave por email na hora, sem cartão nem aprovação manual. Limite de 50 pedidos/minuto, cobre só o território espanhol. Dá para pedir previsão diária por município e avisos meteorológicos oficiais (CAP). **Ainda não integrei** — a app já tem meteorologia mundial via Open-Meteo (sem chave), e os avisos oficiais da AEMET vêm num formato mais trabalhoso de tratar no servidor (XML/CAP dentro de um ficheiro comprimido, não JSON direto) — mas se quiseres avisos oficiais espanhóis especificamente, dá para fazer com essa chave.
...
**Resumindo:** das três, só a AEMET está pronta a integrar com uma chave gratuita simples. Se quiseres que eu avance com isso (ex.: avisos meteorológicos oficiais de Espanha na aba 🌦️ Meteorologia), diz-me que implemento a seguir.
