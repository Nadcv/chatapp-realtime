...
- **Autenticação de usuários** — tela de login/criar conta com nome, telemóvel, país, email e senha. As senhas são guardadas com hash `scrypt` (nunca em texto puro), num arquivo `users.json` no servidor (mesmo padrão de persistência do histórico de mensagens — ver aviso sobre deploys abaixo). O cálculo do hash usa a versão **assíncrona** (`crypto.scrypt`, não `scryptSync`) — o scrypt é deliberadamente pesado em CPU (é isso que o torna seguro contra força bruta), e a versão síncrona bloqueava a única thread de JavaScript do Node inteira durante esse cálculo: um registo ou login congelava o servidor INTEIRO nesse instante, mesmo para quem não estava a fazer login nenhum. Sob muitos registos/logins ao mesmo tempo isso empilhava-se a sério. A versão assíncrona delega o cálculo para a threadpool do Node, sem nunca bloquear a thread principal.
...
## 📱 Validação de telemóvel no registo (cascata Numverify → Veriphone → AbstractAPI)

Além do email, o número de telemóvel passa por uma validação num serviço de "lookup" — confirma se o número tem um formato e prefixo de operadora reais atribuídos (país, operadora, se é móvel ou fixo). Números claramente inventados ou impossíveis são recusados logo no registo, com uma mensagem clara.

- **Três provedores em cascata**, cada um com um plano gratuito pequeno: **Numverify** (apilayer.net) → **Veriphone** (veriphone.io) → **AbstractAPI** (abstractapi.com). Assim que um provedor responde de forma **definitiva** (número válido ou inválido), a cascata pára aí — só se avança para o seguinte quando o atual não está configurado, está sem quota, ou está indisponível (rede/timeout/resposta inesperada). Nunca se avança por causa do número em si.
- **Cada provedor só entra em jogo se a sua variável de chave estiver configurada**: `NUMVERIFY_API_KEY`, `VERIPHONE_API_KEY`, `ABSTRACT_PHONE_API_KEY` (todas opcionais — configura só as que quiseres; com nenhuma configurada, este passo é simplesmente saltado, tal como acontece com o email sem servidor configurado).
...
- **Duas fontes de livros, independentes uma da outra** — um seletor junto à pesquisa alterna entre **Project Gutenberg** (via Gutendex) e **Internet Archive**. Como o Gutendex já bloqueou pedidos em produção mais de uma vez (ver abaixo), ter uma segunda fonte com uma API própria (`archive.org`, sem chave) significa que a Biblioteca continua a funcionar mesmo que uma das duas esteja em baixo.
...
- **Internet Archive** (`/api/library/archive`) usa a API pública `advancedsearch.php` do archive.org para listar livros em português com EPUB disponível (ou pesquisar por título/autor, tal como o Gutendex). O ficheiro em si passa por `/api/library/archive/file?identifier=...` — ao contrário do proxy do Gutenberg, este nunca recebe uma URL do cliente (só um "identifier" validado por uma expressão regular simples), por isso não precisa de nenhuma allowlist anti-SSRF: o próprio servidor consulta `/metadata/{identifier}` para saber qual é o ficheiro EPUB real antes de o descarregar de `/download/{identifier}/{ficheiro}`. Tal como no Gutenberg, os bytes descarregados são verificados pela assinatura ZIP ("PK") antes de serem enviados ao leitor.
...
- A leitura em si usa a biblioteca **epub.js** (carregada via CDN, tal como o Leaflet e o Three.js já usados na app) — que por sua vez precisa do **JSZip** carregado antes dela (a build do epub.js não o traz embutido; sem isto, todos os livros falhavam sempre ao abrir com "JSZip lib not loaded", mesmo com tudo o resto a funcionar). Se o livro não abrir por outro motivo (ficheiro inválido, CDN em baixo, o leitor demorar demasiado a aparecer, etc.), mostra a razão exata em vez de deixar o ecrã preso em branco.
- **"◀ Anterior" / "Seguinte ▶"** (ou as setas ← → do teclado, no computador) viram a página. Sem isto, o livro abria mas ficava preso na primeira página/capa, sem forma nenhuma de continuar a ler.
- **Lista de reserva quando o Gutendex está em baixo**: o Gutendex é um serviço gratuito de terceiros, sem garantia de estar sempre disponível (já devolveu HTTP 503 e depois HTTP 403 em produção — parece ser um bloqueio mais duradouro, não só um pico de tráfego). Quando isso acontece, a Biblioteca nunca fica vazia — mostra sempre uma lista fixa de 9 clássicos em português/brasileiro (Os Lusíadas, Dom Casmurro, Memórias Póstumas de Brás Cubas, O Primo Basílio, Os Maias, A Cidade e as Serras, Viagens na Minha Terra, Amor de Perdição, Iracema — todos com IDs reais e verificados do Project Gutenberg), com um aviso claro de que não é o resultado da pesquisa/lista pedida. O ficheiro destes livros vem diretamente do gutenberg.org (um serviço independente do Gutendex), por isso continua a funcionar mesmo com o Gutendex bloqueado.

## 🔒 Auditoria de segurança formal — 5 vulnerabilidades corrigidas

Uma revisão de segurança dedicada (metodologia: identificação → verificação independente de cada achado → só relatar com confiança ≥8/10) encontrou e corrigiu 5 vulnerabilidades reais neste código, acumulado organicamente ao longo de muitas funcionalidades:

- **Falsificação de identidade por Socket.IO (crítico)** — o servidor confiava cegamente no telefone que o cliente dizia ter no evento `user_login`, sem validar contra nenhuma sessão real. Isto permitia a qualquer pessoa (bastava saber um número de telefone, nem sequer a senha) ligar um socket.io "cru" e ser tratada como dona dessa conta em tudo — ler conversas privadas, e mesmo sequestrar a conta por completo (mudar o email de recuperação via `set_email`, depois usar "esqueci a senha" normal). Corrigido: o telefone agora só vem de `sessions[token]` — o mesmo token de sessão emitido por `/api/login`/`/api/register` e já usado em todas as rotas HTTP — nunca do que o cliente afirma ser.
- **XSS armazenado (alto impacto)** — o nome de exibição da conta (sem qualquer restrição de caracteres no registo) e as citações de resposta apareciam em vários sítios (lista de conversas, nome do remetente numa mensagem, threads, resultado de pesquisa de utilizador) inseridos diretamente em HTML sem escapar — uma conta registada com `<img src=x onerror="...">` como nome executava JavaScript no browser de qualquer pessoa que a visse, o que incluía qualquer vítima de um primeiro DM (que já adiciona o remetente aos contactos automaticamente). Corrigido aplicando `escapeHtml()` (já usado consistentemente noutros sítios do código) em cada um destes pontos.
- **SSRF em `/api/news/read`** — aceitava qualquer URL, incluindo endereços internos/privados, sem validação nenhuma, e devolvia o conteúdo lido de volta a quem pediu.
- **SSRF por redirecionamento em `/api/link-preview`** — validava o endereço da URL original, mas seguia (`redirect: 'follow'`) qualquer redirecionamento HTTP sem validar o destino final; um site com um endereço válido que respondesse com um redirecionamento para um endereço interno contornava a proteção por completo.
- **SSRF em `/api/gemini-chat`** — ficheiros remotos anexados a uma conversa com o Gemini eram descarregados a partir de qualquer URL http(s), sem nenhuma restrição de destino.
