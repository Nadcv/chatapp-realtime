...
**Nota sobre a queixa de vídeo numa chamada 1-para-1**: revi a fundo o código de sinalização (troca de ofertas/respostas, fila de candidatos ICE, recuperação automática se a ligação cair, aviso para desbloquear o som/vídeo em navegadores mais restritos como o Safari/iPhone) e está bem protegido contra as causas mais comuns deste tipo de problema. Não consegui reproduzir uma falha específica de "só um lado mostra imagem" só a partir da leitura do código — isto precisaria de um teste real entre dois aparelhos/redes diferentes para isolar a causa (ex.: um dos lados numa rede mais restrita, ou o serviço gratuito de retransmissão TURN usado — openrelay.metered.ca — a falhar nesse momento, o que está fora do controlo do código da app). Se o problema persistir, ajuda saber: em que aparelho/rede acontece, se o som funciona mas o vídeo não (ou o contrário), e se acontece sempre a chamar ou sempre a atender.
