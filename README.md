...
A CP continua sem publicar posição ao vivo dos comboios (ver acima) — mas o horário GTFS já nos diz exatamente a que horas cada comboio passa por cada estação. Com o interruptor "🚆 Ver comboios da CP em trânsito", o servidor calcula uma **posição estimada** para cada viagem atualmente a decorrer: interpola linearmente entre as duas paragens do horário que envolvem a hora atual (ou mostra a paragem exata, se o comboio estiver parado nela dentro da janela chegada→partida).

**Isto não é GPS real** — assume que a viagem está a decorrer exatamente conforme o horário, sem atrasos nem paragens não planeadas. Cada comboio no mapa tem um ícone (🚂) com popup ("posição estimada pelo horário — não é GPS ao vivo") e um link **"🔎 Ver detalhes deste comboio"**. Ao selecionar um comboio (só um de cada vez, para não sobrepor rotas quando há vários em trânsito): aparece a linha tracejada da rota completa dessa viagem, um marcador por paragem (🟢 origem, 🔸 intermédia, 🔴 destino) com a hora do horário, e um painel com o itinerário completo — paragens já passadas riscadas (✅), a atual a negrito (🚂), as seguintes normais. Atualiza a cada 30 segundos enquanto o interruptor estiver ligado (cálculo local, sem custo de pedidos a nenhuma API externa). Endpoint: `/api/trains/positions-estimated`.
