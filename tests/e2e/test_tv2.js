const { chromium } = require('playwright');

(async () => {
  const browser = await chromium.launch({ executablePath: '/opt/pw-browsers/chromium' });
  const page = await browser.newPage();
  page.on('console', msg => { if (msg.type() === 'error') console.log('PAGE ERROR:', msg.text()); });

  await page.goto('http://localhost:3000');
  await page.click('.login-switch');
  const ts = Date.now();
  await page.fill('#regName', 'TV Teste2');
  await page.fill('#regUsername', 'tv_test2_' + ts);
  await page.fill('#regPhone', '+3516' + ts.toString().slice(-8));
  await page.selectOption('#regCountry', 'Portugal');
  await page.fill('#regEmail', 'tvtest2' + ts + '@test.com');
  await page.fill('#regPassword', 'senha123');
  await page.click('button:has-text("Criar conta")');
  await page.waitForSelector('#mainApp', { state: 'visible', timeout: 8000 });

  await page.click('#mediaBtn');
  await page.click('#modalMediaFeatures button[onclick*="openTvScreen"]');
  await page.waitForSelector('#tvScreen.active');
  await page.waitForTimeout(500);

  console.log('Category tabs:', (await page.textContent('#tvCategoryTabs')).replace(/\s+/g, ' ').trim());
  console.log('Channel tabs (news):', (await page.textContent('#tvTabs')).replace(/\s+/g, ' ').trim());
  console.log('Initial iframe src:', await page.getAttribute('#tvFrame', 'src'));

  await page.click('#tvCategoryTabs button:has-text("Filmes")');
  await page.waitForTimeout(300);
  console.log('Channel tabs (filmes):', (await page.textContent('#tvTabs')).replace(/\s+/g, ' ').trim());
  console.log('Iframe src (filmes):', await page.getAttribute('#tvFrame', 'src'));

  await page.click('#tvCategoryTabs button:has-text("Jogos")');
  await page.waitForTimeout(300);
  console.log('Channel tabs (jogos):', (await page.textContent('#tvTabs')).replace(/\s+/g, ' ').trim());
  console.log('Iframe src (jogos):', await page.getAttribute('#tvFrame', 'src'));

  await page.click('#tvCategoryTabs button:has-text("Notícias")');
  await page.waitForTimeout(300);
  console.log('Back to news, iframe src:', await page.getAttribute('#tvFrame', 'src'));

  await browser.close();
})().catch(e => { console.error('TEST FAILED:', e); process.exit(1); });
