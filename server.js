...
if (MONGO_URI) {
  mongoose.connect(MONGO_URI)
    ...
    .catch(err => console.error('Erro ao ligar ao MongoDB Atlas:', err));
} else {
  console.log('⚠️ AVISO: MONGO_URI não definida. A usar fallback de ficheiros locais.');
}
...
function saveUsers() {
  if (isDbConnected) {
    // Sincronização assíncrona para a base de dados é feita pontualmente nos registos
    return;
  }
...
async function loadDataFromMongo() {
  try {
    const dbAccounts = await AccountModel.find({});
    dbAccounts.forEach(acc => {
      accounts[acc.phone] = acc.toObject();
      if (!firstRegisteredPhone) firstRegisteredPhone = acc.phone;
    });

    const dbGroups = await GroupModel.find({});
    dbGroups.forEach(g => {
      groups[g.id] = g.toObject();
    });

    const dbMsgs = await MessageModel.find({}).sort({ createdAt: 1 });
    dbMsgs.forEach(m => {
      if (!messagesByRoom[m.chatId]) messagesByRoom[m.chatId] = [];
      messagesByRoom[m.chatId].push(m.toObject());
    });
    console.log('🔄 Dados carregados da base de dados com sucesso.');
  } catch (err) {
...
app.post('/api/register', async (req, res) => {
  ...
  if (isDbConnected) {
    try {
      ...
      console.error('Erro ao gravar utilizador no Mongo:', e);
    }
  } else {
    saveUsers();
  }
...
app.post('/api/publish-key', async (req, res) => {
  ...
  if (isDbConnected) {
    await AccountModel.updateOne({ phone }, { publicKey: accounts[phone].publicKey });
  } else {
    saveUsers();
  }
  ...
  socket.on('send_message', async (data) => {
    ...
    if (isDbConnected) {
      try {
        ...
        console.error('Erro ao guardar mensagem na base de dados:', e);
      }
    } else {
      saveMessagesLocal();
    }
